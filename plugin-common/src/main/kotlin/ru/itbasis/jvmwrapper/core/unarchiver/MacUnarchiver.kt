...
class MacUnarchiver(sourceFile: File, targetDir: File, stepListener: ProcessStepListener? = null) :
  ...
  private fun unpackCpio(dir: File) {
    dir.listFiles { _, name -> name?.contains("jdk") ?: false }.forEach { jdkDir ->
      ProcessBuilder("cpio", "-i").redirectInput(jdkDir.resolve("Payload")).directory(dir).start().run()
    }
  }
  ...
  private fun detach(quiet: Boolean = false) = runtime.exec(arrayOf(
    CMD_HDIUTIL, "detach", volumePath, "-force"
  ).apply {
    if (quiet) this.plus("-quiet")
  })

  private fun findPkgFile() =
    File(volumePath).walkTopDown().find { it.extension == "pkg" } ?: throw IllegalStateException("package file not found in '$volumePath'")

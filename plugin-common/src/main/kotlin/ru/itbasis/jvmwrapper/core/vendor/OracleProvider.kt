...
class OracleProvider(private val jvmVersion: JvmVersion) : AbstractProvider() {
  ...
  override val remoteArchiveFile: RemoteArchiveFile
    get() {
      return latestDownloadPageUrl.getRemoteArchiveFile() ?: archiveDownloadPageUrl.getRemoteArchiveFile()
  ...
  override fun download(remoteArchiveFile: RemoteArchiveFile, target: File, downloadProcessListener: DownloadProcessListener?) {
    ...
    target.outputStream().use { fos ->
      entity.content.buffered().copyTo(target = fos, listenerStep = sizeTotal / 10_000) { sizeCurrent ->
        return@copyTo downloadProcessListener?.invoke(remoteArchiveFile.url, sizeCurrent, sizeTotal) == true
      }
      fos.flush()
    }
  }
  ...
  private val archiveDownloadPageUrl: String?
    get() {
      val previousReleasesPage = regexPreviousReleasesPage.find(indexPage)?.groupValues?.get(1)?.htmlContent()

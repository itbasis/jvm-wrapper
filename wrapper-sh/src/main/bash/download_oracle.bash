...
function oracle_otn_curl_send_request() {
	...
	curl ${CURL_OPTS} -A "${OTN_USER_AGENT}" -D "${OTN_HEADERS_FILE}" -b "${OTN_COOKIE_FILE}" -c "${OTN_COOKIE_FILE}" ${CURL_OPTIONS} "${url}" >"${OTN_CONTENT_FILE}"
}
...
function oracle_download_jdk() {
	...
	curl ${CURL_OPTS} -A "${OTN_USER_AGENT}" -b "${OTN_COOKIE_FILE}" -o "${ARCHIVE_FILE}" --cookie "oraclelicense=accept-securebackup-cookie" "${ARCHIVE_JVM_URL}"
...
function oracle_check_the_need_for_downloading() {
	...
	debug "JVM_HOME_DIR=${JVM_HOME_DIR}"
...
function oracle_otn_page_latest_version_jdk_page_parser() {
	local -r index_page=$(curl -sL https://www.oracle.com/technetwork/java/javase/downloads/index.html)
	local awk_mask='name=\"JDK'${JVM_VERSION_MAJOR}'\"[[:space:]]href=\"[^\"]*'
	# shellcheck disable=SC2155
	local row=$(echo "${index_page}" | awk '{ if (match($0, "'"${awk_mask}"'")) print substr($0, RSTART, RLENGTH) }')
	awk_mask='href=\".*'
	row=$(echo "${row}" | awk '{ if (match($0, "'"${awk_mask}"'")) print substr($0, RSTART+6, RLENGTH-6) }')
	JVM_PAGE_URL=https://www.oracle.com${row#//www.oracle.com}

	if [[ "x${JVM_PAGE_URL}" != "x" ]]; then
		oracle_otn_page_archive_jdk_parser
		if [[ "x${ARCHIVE_JVM_URL}" != "x" ]]; then
			return 0
		fi
	fi

	local -r url_archive=https://www.oracle.com$(curl -sSL https://www.oracle.com/technetwork/java/javase/downloads/index.html | awk 'match($0, /(\/technetwork\/java\/javase\/archive-[^\"]+)/) { print substr($0, RSTART, RLENGTH) }' | head -1)

	JVM_PAGE_URL=https://www.oracle.com$(curl -sSL "${url_archive}" | awk 'match($0, /(\/technetwork\/java\/javase\/downloads\/java-archive-[^'"${JVM_VERSION_MAJOR}"']+'"${JVM_VERSION_MAJOR}"'-[^\"]+)/) { print substr($0, RSTART, RLENGTH) }' 2>&1 | head -1)

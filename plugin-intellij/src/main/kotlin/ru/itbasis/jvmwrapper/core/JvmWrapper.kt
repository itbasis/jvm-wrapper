...
class JvmWrapper(
  ...
  private val homeDir: File = SystemUtils.getUserHome().resolve(".jvm")
  ...
  val jvmHomeDir: File = homeDir.resolve(jvmName)
    .also { jvmHomeDir ->
    ...
    .run { if (SystemInfo.isMac) this.resolve("Home") else this }
    .apply { check(isDirectory, { "jvm home directory is not exists: $this" }) }

  companion object {
    const val SCRIPT_FILE_NAME = "jvmw"
    const val JVMW_PROPERTY_FILE_NAME = "$SCRIPT_FILE_NAME.properties"
    val DEFAULT_PROPERTIES: JvmWrapperProperties =
